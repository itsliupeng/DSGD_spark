...
object CFMatrixPartition {
  ...
  def batchUpdate(WW: Map[Int, Array[Double]], HH: Map[Int, Array[Double]], ratingByW: Map[Int, Seq[(Int, Double)]], ratingByH: Map[Int, Seq[(Int, Double)]], numFactor: Int) = {
    ...
    WW.foreach { case (i, wi) =>
      ...
      if (rating.nonEmpty) {
        ...
        blas.dgemv("N", Hx, Hy, -alpha / Hy, H, Hx, V, 1, coff, wi, 1)
      }
    }
    HH.foreach { case (j, hj) =>
  ...
  def main(args: Array[String]): Unit = {
    ...
    val numPart = 1
    val ratingByHSeq: immutable.Seq[Map[Int, Seq[(Int, Double)]]] = 0.until(numPart).map (index => ratingByH.mapValues(s => s.filter { case (i, _) => i % numPart == index }))
    ...
    for (i <- 0.until(numIter)) {
      ...
      for (j <- 0.until(numPart)) {
        batchUpdate(WW, HH, ratingByW, ratingByHSeq(j), numFactor)
      }
      val error = evaluation(rating, WW, HH)
      val timeElapsed = System.currentTimeMillis() / 1000 - start
      totalTime += timeElapsed
      println(s"loss of iter $i in numPartition $numPart time ${timeElapsed} totalTime ${totalTime} = ${error}")
    }
